...
/*         Header header;
        ...
        Header header;
        // 前4位存大小
        header.package_size = (array[2] << 16) | (array[1] << 8) | array[0];
        // 后4位存类型
        header.type = static_cast<BODYTYPE>(array[3]);

        //std::cout<<"package_size:"<<header.package_size <<" type:"<<header.type<<std::endl;
        return header; 
    }
    ...
    {
        Net::UserMoney info;
        info.set_userid(std::stoi(username));
