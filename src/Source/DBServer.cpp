...
void DBServer::HandleUserLogin(Net::LoginData &data, int fd)
{
    ...
    response = ProtoUtil::SerializeLoginResponseToArray(ret, msg, resp_length, data.username(), ret, resp_opt);
    if (response != nullptr)
    {
...
int DBServer::QueryUser(std::string username, std::string password)
{
    ...
    {
        int ret = 0;
        std::string pwdandid = std::string(reply->str);
        size_t pos = pwdandid.find_last_of(":");
        if(pos != std::string::npos){
            std::string first = pwdandid.substr(0, pos);
            std::string second = pwdandid.substr(pos + 1);
            if (first == password)
            {
                ret = std::stoi(second);
            }
    ...
    {
        std::cout<<"id:"<<atoi(row[0])<<std::endl;
        ret = atoi(row[0]);
    }
    ...
    if (ret)
    {
        redisReply *reply = (redisReply *)redisCommand(redis, "set %s %s", key.c_str(), (password+":" + std::to_string(ret)).c_str());
        if (reply != nullptr)
        {
            // 释放 reply 对象
            freeReplyObject(reply);
...
int DBServer::InsertUser(std::string username, std::string password)
{
    int ret = 0;
    if (mysql == nullptr)
    {
    ...
    unsigned int new_id = mysql_insert_id(mysql); // 获取刚刚插入记录的 ID 值
    // 执行 SQL 语句获取
    ret = new_id;
