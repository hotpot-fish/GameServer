...
void ServerBase::HandleReceivedMsg(RingBuffer *buffer, int fd)
{
    ...
    while (buffer_size >= HEAD_SIZE)
    {
        ...
        {
            uint32_t body_length = h.package_size;
...
int ServerBase::SendMsg(BODYTYPE type, size_t totalSize, const uint8_t *data_array, int fd)
{
    ...
    {
        size_t tmpSize = totalSize;
        uint8_t resp[totalSize + HEAD_SIZE];

        for (int i = 0; i < 3; i++) {
            resp[i] = (uint8_t)(totalSize & 255);
            totalSize = totalSize >> 8;
        }       
        resp[3] = (uint8_t)(type);

        for (int i = HEAD_SIZE; i < tmpSize + HEAD_SIZE; i++)
        {
...
void ServerBase::BootFrameServer(int port , int center_server_client)
{
